...
export const addProduct = async (name: string, price: number, description: string, file: string  | ArrayBuffer, category: string, accessToken: string) => {
    return await axios.post('/products/addProduct', {name, price, description, file, category} ,{headers: { ContentType: 'application/json', Authorization: `Bearer ${accessToken}` }, withCredentials: true});
};

export const getProducts = async (sortState: string, filterState: string) => {
    return await axios.get(`/products/products?sort=${sortState}&filter=${filterState}`, {headers: {"Access-Control-Allow-Origin": "*"}})
...
export const deleteProducts = async (productsIds: Array<string>, accessToken: string) => {
    return await axios.post('/products/delete', {products: productsIds}, {headers: { ContentType: 'application/json', Authorization: `Bearer ${accessToken}` }, withCredentials: true})
        .then(res => ({
            data: res.data,
            status: res.status,
        }))
